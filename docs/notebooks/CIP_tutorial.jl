...
If you did not prepare your sea-level curve, we provide one and you could download it from emails. You could download it to your own computer, and replace the PTH with the real pathway that you stored the txt file.
...
md"""
If we assume we can find sufficient forams (i.e., ignore sampling bias) to extract O isotopes. What kind of oxygen isotope curve did you expect?

First we construct a pseudo Oxygen isotope curve based on the sea-level curve (e.g.,assuming low-sea-level is glaciation and high O isotope composition). We do not consider the offset of phases.

We then use the ADM extracted from the carbonate platform to see, to test how much of the original signals would be preserved. Could the preserved signals still refect all of the orbital cycles? 
...
let
	...
	sl_magnitude = (max(sea_level) - min(sea-level)) ./ 1.0u"m"
	d18O_original = sea_level ./ sl_magnitude .* O_magnitude
	
	function determine_preservation(ADM::Vector)
		ADM_diff = diff(ADM)
		preservation_potential = Float64[]
			for i in 1:length(ADM_diff)
				if ADM_diff[i] > 0.0001u"m" #arbitrary
	...
	function d18O_preserve(preservation_potential::Vector,d18O::Vector)
		preservation_potential .* d18O
	end
	...
	d180_shallow = d18O_preserve(determine_perservation(adm_shallow), d18O_original[2:end])
	d180_deep = d18O_preserve(determine_perservation(adm_deep), d18O_original[2:end])
...
manifest_format = "2.0"
project_hash = "472247da1feb506fc4bdaedfaa9d620974315a3c"
