...
# ╔═╡ 5c253fa0-8ed2-4f19-a3f6-01444bdd0926
md"""
## Output selection

A CarboKitten run can generate large output files. You can select what parts of a run to save.
"""

# ╔═╡ 4fc36c1a-3c6c-41b0-bc7f-69ea7a9118e9
output_spec = Dict(
	:topography => OutputSpec(write_interval=100),
	:profile => OutputSpec(slice=(:, 25))
)

# ╔═╡ 9ae63bf8-15f3-4f82-b704-5e90528684a4
md"""
This selects one in 100 2d slices to inspect the full topography of the model, and also a profile along the x-axis with complete time information.
"""
...
# ╔═╡ d05a4cde-7661-4625-9ae5-697fcdef7a89
md"""We first read the data that we selected to be written to the `profile` group. This was a slice of the full data, so we use the `read_slice` function (there are also `read_volume` for the full data, and `read_column` for data columns)."""

# ╔═╡ ec4f0021-12af-4f8c-8acb-970e6820d2a4
#=╠═╡
header, profile_data = read_slice(tutorial_output, :profile)
  ╠═╡ =#

# ╔═╡ e80aaf02-c5bf-4555-a82d-b09dcf785381
md"""
We can now select columns from the data slice by selecting. We'll choose the 11th, 26th and 41th grid in the direction towards to the deep sea. That is to say, the 11th is proximal to the land while the 41th is distal. Given that each grid is with fixed size of 300 m × 300 m, this indicates that we are extracting information from locations at 3 km, 7.5 km and 12 km away from the land. 
"""

# ╔═╡ 137774e6-5643-4e98-8cdb-7e580c01e0ee
export_locations = [11, 26, 41]

# ╔═╡ 10035792-8208-4bef-9452-f54109aa3d97
#=╠═╡
columns = [profile_data[i] for i in export_locations]
  ╠═╡ =#
...
let
	...
	for i = 1:3
		xpos = uconvert(u"km", xaxis[export_locations[i]])
...
let
	...
	for (i, loc) in enumerate(export_locations)
		xpos = uconvert(u"km", xaxis[loc])
		ax = Axis(fig[1,i], title="$(xpos) offshore", ylabel="height (m)", xticksvisible=false, xtickformat="", width=70)
		stratigraphic_column!(ax, header, columns[i])
	end
...
let
	...
	ypos = yaxis[profile_data.slice[2]]
	sediment_profile!(ax, header, profile_data)
...
manifest_format = "2.0"
project_hash = "dea03a584a2918f70ada12101be17ceb687313be"
...
git-tree-sha1 = "cb0131921789ba8fdcb200728bc2e3ea70e87e71"
