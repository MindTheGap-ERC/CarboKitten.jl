...
We are solving sediment transport using finite difference methods. Our active layer approach to transport can be written as a diffusion equation in terms of the sediment concentration,
...
where $v(w)$ is the wave transport velocity and $s(w)$ the wave transport shear.
