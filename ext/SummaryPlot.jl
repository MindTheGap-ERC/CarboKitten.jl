...
function summary_plot(fid::HDF5.File; wheeler_smooth=(1, 1), show_unconformities=true, facies_colors=nothing, facies_names=nothing)
    ...
    volume_data = read_volume(fid[data_groups[:volume][end]])
    ...
    # glamour_view now handles compacted automatically
...
section_data = if length(data_groups[:slice]) == 0
    ...
    volume_data === nothing && error("No volume data available to extract section.")

    thickness = if hasproperty(volume_data, :compacted_thickness)
        volume_data.compacted_thickness
    else
        volume_data.sediment_thickness
    end

    y_slice = div(size(thickness, 2), 2) + 1
    volume_data[:, y_slice]
...
facies_names = if haskey(fid["input"], "facies")
    map(x -> String(x), fid["input"]["facies"])
else
    string.(1:n_facies)
end
...
sm, df = wheeler_diagram!(
    ...
    ax5 = Axis(fig[2,3], title = "production curves")
