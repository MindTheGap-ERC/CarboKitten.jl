...
using ..Vectors
using ..BoundaryTrait
using ..Config: Box, AbstractBox, centered_axes
...
function offset(box::AbstractBox{Reflected{2}}, a::Vec2, Δa::Vec2)
    ...
    (x=clip(a.x + Δa.x, 0.0, box.phys_size.x), y=clip(a.y + Δa.y, 0.0, box.phys_size.y))
end

function offset(box::AbstractBox{Periodic{2}}, a::Vec2, Δa::Vec2)
    (x=mod(a.x + Δa.x, box.phys_size.x), y=mod(a.y + Δa.y, box.phys_size.y))
end
...
function grid_sample(box::AbstractBox{BT}, density::AbstractMatrix{Float64}, sub_sample::Int) where {BT<:Boundary{2}}
    phys_scale = box.phys_scale / u"m" |> NoUnits
    Channel{Tuple{Vec2,Float64}}() do ch
        for (idx, mass) in pairs(density)
            subgrid_spacing = 1.0 / sub_sample
            subgrid_axis = (subgrid_spacing/2):subgrid_spacing:1.0
            subgrid = Iterators.product(subgrid_axis, subgrid_axis)
            for dx in subgrid
                p = (x=(idx[1] - 1 + dx[1]) * phys_scale, y=(idx[2] - 1 + dx[2]) * phys_scale)
                put!(ch, (p, mass / sub_sample^2))
            end
        end
    end
end
