...
function calculate_ceq(chemparam, cli::climate, facies::Facies)
    ceq = (cli.pco2 .* (chemparam[K1] .* chemparam[K2] .* chemparam[KH]) ./(4 * chemparam[K2] .* chemparam[gama_Ca] .* (chemparam[gama_alk]).^2)).^(1/3)
...
function dissolution(cli::climate, water_depth::Float64, chemparam,  facies::Facies)
    ...
    lambda = cli.P .* facies.inf ./ (chemparam[a] .* facies.L)
